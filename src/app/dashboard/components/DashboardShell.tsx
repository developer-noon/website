...
export default function DashboardShell({ children, user }: { children: React.ReactNode; user: DashboardUser }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#111827]">
      <div className={`fixed inset-0 z-50 bg-[#111827]/20 transition lg:hidden ${isSidebarOpen ? 'visible opacity-100' : 'pointer-events-none invisible opacity-0'}`} onClick={() => setIsSidebarOpen(false)} aria-hidden="true" />
      <aside className={`fixed inset-y-0 left-0 z-50 w-[248px] border-r border-[#E5E7EB] bg-white px-4 py-5 transition-transform lg:block lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="mt-6 space-y-1" aria-label="Workspace navigation">
          <SidebarLink href="/dashboard/newsletter?tab=theme" icon={<EnvelopeIcon className="h-[18px] w-[18px]" aria-hidden="true" />} label="Brand theme" active={activeTab === 'theme'} onNavigate={() => setIsSidebarOpen(false)} />
          <SidebarLink href="/dashboard/newsletter?tab=sessions" icon={<EnvelopeIcon className="h-[18px] w-[18px]" aria-hidden="true" />} label="Sessions" active={activeTab === 'sessions'} onNavigate={() => setIsSidebarOpen(false)} />
          <SidebarLink href="/dashboard/newsletter?tab=submissions" icon={<EnvelopeIcon className="h-[18px] w-[18px]" aria-hidden="true" />} label="Submissions" active={activeTab === 'submissions'} onNavigate={() => setIsSidebarOpen(false)} />
          <SidebarLink href="/dashboard/newsletter?tab=newsletter" icon={<EnvelopeIcon className="h-[18px] w-[18px]" aria-hidden="true" />} label="Newsletter" active={activeTab === 'newsletter'} onNavigate={() => setIsSidebarOpen(false)} />
        </nav>
      </aside>
      <header className="sticky top-0 z-40 border-b border-[#E5E7EB] bg-white/95 backdrop-blur-xl lg:ml-[248px]">
        <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 lg:hidden"><button type="button" onClick={() => setIsSidebarOpen(true)} className="rounded-lg border border-[#D1D5DB] p-2 text-[#374151]" aria-label="Open workspace navigation"><Bars3Icon className="h-5 w-5" aria-hidden="true" /></button><span className="text-sm font-semibold text-[#111827]">Newsletter</span></div>
          <div className="hidden text-sm font-semibold text-[#6B7280] lg:block">Newsletter workspace</div>
